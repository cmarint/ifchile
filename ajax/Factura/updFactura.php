		...
		if(!empty($postdata)){	
		    ...
		    else
		    {
				...
				if( $request->estadoUpd ==1)//Update desde Publicadas
				{
					...
					$query="UPDATE factura SET Monto='$request->Monto',Emisor='".htmlspecialchars($request->Emisor)."',Glosa='$request->Glosa',Receptor='".htmlspecialchars($request->Receptor)."',Descuento=$request->Descuento,UtilidadEsperada=$request->UtilidadEsperada,UtilidadReal=null,FechaPago=null,FechaVencimiento='$FchVenc',PlazoPago=null,DiasPago=$request->DiasPago,ImagenFactura='$request->ImagenFactura',DescuentoSeguro=$request->DescuentoSeguro,UtilidadEsperadaSeguro=$request->UtilidadEsperadaSeguro,Comentario='$request->Comentario',FechaExpiracion='$FchExp' WHERE Id= $request->Id";
